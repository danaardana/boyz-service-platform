...
class AdminController extends Controller
{
    public function __construct()
    {
        ...
        View::composer('admin.*', function($view) {
            ...
            $recentMessages = ContactMessage::orderBy('zxc_ca', 'desc')
                ->take(5)
                ->get();
            $view->with([
    ...
    {        
        ...
        $admins = Admin::orderBy('wer_ca', 'desc')->get();
    ...
    {      
        $admin = \App\Models\Admin::findByEmail($email);
    ...
    public function storeAdmin(Request $request)
    {
        ...
        $request->validate([
            'name' => 'required|string|max:255',
            'email' => 'required|string|email|max:255|unique:xxy_k7m2p,rty_em',
            'password' => 'required|string|min:8|confirmed',
            'role' => 'required|in:owner,marketing,support',
            'is_active' => 'boolean',
        ...
        try {
            ...
            $adminData = [
                ...
                'role' => $request->role, // logical → encrypted automatically
    ...
    public function updateAdmin(Request $request, Admin $admin)
    {
        try {
            $request->validate([
                ...
                'email' => 'required|string|email|max:100|unique:xxy_k7m2p,rty_em,' . $admin->k1_id . ',k1_id',
                'password' => 'nullable|string|min:8|confirmed',
                'is_active' => 'boolean',
                'verified' => 'boolean',
            ], [
    ...
    public function checkEmailAvailability(Request $request)
    {
        ...
        $existingAdmin = Admin::findByEmail($request->email);

        return response()->json([
            'exists' => !is_null($existingAdmin),
            'available' => is_null($existingAdmin)
        ]);
    }

    public function accessLogsPage()
    {
        $accessLogs = Notification::getAccessLogs();

        return view('admin.access-logs', compact('accessLogs'));
    }

    public function activityLogsPage()
    {
        $activityLogs = Notification::getActivityLogs();
    ...
    public function systemHealthPage()
    {
        $widgets =
        [
            'error' => Notification::where('tya_tp', 'system')->count(),
            'critical' => Notification::where('tya_tp', 'warning')->count(),

        ];

        $cursor = Carbon::create(2025, 10, 1)->startOfWeek();
        $endOfRange = Carbon::create(2026, 2, 5)->endOfDay();
        ...
        foreach ($weeks as $week) {
            ...
            // === ACCESS = semua activity ===
            $access = Notification::whereBetween('ujm_ca', [$start, $endW])
                ->count();

            // === ERROR = login failure (sementara) ===
            $error = Notification::where('tya_tp', 'login_failure')
                ->whereBetween('ujm_ca', [$start, $endW])
                ->count();

            $labelsErrorTrendChart[]     = $start->format('d M') . ' – ' . $endW->format('d M');
            $accessData[] = $access;
            $errorData[]  = $error;
        }

        $errorTrendChart = [
            'labels' => $labelsErrorTrendChart,
            'access' => $accessData,
            'error'  => $errorData,
        ];
        

        $labelsLoginChart = $weeks->map(fn ($w) =>
        ...
        foreach ($weeks as $week) {
            ...
            $success = Notification::where('tya_tp', 'login')
                ->whereBetween('ujm_ca', [$start, $end])
                ->count();

            $failure = Notification::where('tya_tp', 'login_failure')
                ->whereBetween('ujm_ca', [$start, $end])
                ->count();

            $loginSuccess[] = $success;
            $loginFailure[] = $failure;
            $totalLogin[]   = $success + $failure;
        }

        $loginChart = [
            'labels'  => $labelsLoginChart,
        ...
        $systemResponses = Notification::query()
            ->whereNotIn('tya_tp', ['login', 'logout'])
            ->orderByDesc('ujm_ca')
            ->limit(10)
            ->get();
        ...
        $healthChart = [
            'error' => Notification::where('tya_tp', 'system')->count(),
            'warning' => Notification::where('tya_tp', 'warning')->count(),
            'info' => Notification::where('tya_tp', 'info')->count(),
            'security' => Notification::where('tya_tp', 'security')->count(),
        ];

        return view('admin.system-health', compact('errorTrendChart', 'widgets','loginChart', 'systemResponses', 'healthChart'));
    }
