    ...
    public function getStats(Request $request): JsonResponse
    {
        try {
            $stats = [
                ...
                'today' => Notification::whereDate('ujm_ca', today())->count(),
                'this_week' => Notification::whereBetween('ujm_ca', [now()->startOfWeek(), now()->endOfWeek()])->count(),
                'by_type' => Notification::selectRaw('tya_tp as type, COUNT(*) as count')
                    ->groupBy('tya_tp')
                    ->pluck('count', 'type')
                    ->toArray(),
                'by_user_type' => Notification::selectRaw('plm_ut as user_type, COUNT(*) as count')
                    ->groupBy('plm_ut')
