...
class CustomerController extends Controller
{
                ...
                'customer' => [
                    ...
                    'contact_messages' => $customer->contactMessages->map(function($msg) {
                        return [
                            ...
                            'subject' => $msg->ikm_ck,
                            'status' => $msg->pkm_st,
                            'category' => $msg->qwe_cat,
                            'created_at' => $msg->created_at ? $msg->created_at->format('M d, Y H:i') : 'N/A'
                        ];
                    }),
                    'chat_conversations' => $customer->chatConversations->map(function($conv) {
                        return [
                            'id' => $conv->id,
                            'status' => $conv->sdw_st,
                            'initial_message' => \Str::limit($conv->cvb_iq ?? '', 100),
                            'created_at' => $conv->created_at ? $conv->created_at->format('M d, Y H:i') : 'N/A'
                        ];
                    })
                ]
            ]);
    ...
    {
        $validator = Validator::make($request->all(), [
            'customer_id' => 'required|exists:mno_9x8w,k2_id',
            'subject' => 'required|string|max:255',
            'message' => 'required|string',
        ]);
    ...
    public function getStats()
    {
        ...
        $customersWithEmail = Customer::whereNotNull('klj_em')->count();
        $customersWithPhone = Customer::whereNotNull('qaz_ph')->count();
        
        // Recent customers (last 30 days)
        $recentCustomers = Customer::where('edc_ca', '>=', now()->subDays(30))->count();
