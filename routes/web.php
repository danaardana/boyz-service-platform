...
Route::prefix('admin')->group(function () {
    ...
    Route::middleware('guest:admin')->group(function () {
        ...
        // Registration Routes (for development)
        Route::get('/register', [AuthController::class, 'showRegisterForm'])->name('admin.register');
        Route::post('/register', [AuthController::class, 'register'])->name('admin.register.submit');
        Route::delete('/delete-all', [AuthController::class, 'deleteAll'])->name('admin.delete.all');
    });
    ...
    Route::middleware(['auth:admin', \App\Http\Middleware\PreventBackHistory::class, \App\Http\Middleware\AdminVerificationMiddleware::class])->group(function () {
        ...
        Route::get('/starter', [AdminController::class, 'starter'])->name('admin.starter');
        ...
        */
        ...
        Route::get('/api/messages/notifications', [ContactController::class, 'getMessageNotifications'])
        ->name('admin.api.messages.notifications');

        Route::prefix('notifications')->group(function () {
        Route::get('/', [\App\Http\Controllers\NotificationController::class, 'getRecentNotifications'])
        ...
        */
        Route::middleware([
            \App\Http\Middleware\RoleMiddleware::class . ':owner'
        ])->group(function () 
        {
            ...
            Route::prefix('admins')->group(function () 
            {
                ...
                Route::get('/{admin}/edit', [AdminController::class, 'editAdmin'])
                ...
                Route::post('/{admin}/verify', [AdminController::class, 'verifyAdmin'])
                    ->name('admin.admins.verify');

                Route::post('/{admin}/activate', [AdminController::class, 'activateAdmin'])
                    ->name('admin.admins.activate');

                Route::post('/{admin}/deactivate', [AdminController::class, 'deactivateAdmin'])
                    ->name('admin.admins.deactivate');
            });

            // Email routes
            Route::prefix('emails')->group(function () 
            {
                Route::post('/reactivation', [EmailController::class, 'sendReactivationNotification'])
                    ->name('admin.emails.reactivation');

                Route::post('/security-code', [EmailController::class, 'sendSecurityCode'])
                    ->name('admin.emails.security-code');

                Route::post('/verification', [EmailController::class, 'sendVerification'])
                    ->name('admin.emails.verification');

                Route::post('/bulk', [EmailController::class, 'sendBulkEmails'])
                    ->name('admin.emails.bulk');
            });      
            
            Route::get('/email-verification/{email}', [AdminController::class, 'emailVerification'])
                ->name('admin.email-verification');

            Route::get('/email-confirmation', [AdminController::class, 'emailConfirmation'])
            ...
            Route::get('/documentation', [AdminController::class, 'documentationPage'])
            ...
            {
                Route::get('/', [App\Http\Controllers\DocumentationController::class, 'index'])
                    ->name('admin.documentation.index');

                Route::get('/search', [App\Http\Controllers\DocumentationController::class, 'search'])
                    ->name('admin.documentation.search');

                Route::get('/{system}', [App\Http\Controllers\DocumentationController::class, 'show'])
                    ->name('admin.documentation.show');

                Route::get('/{system}/export', [App\Http\Controllers\DocumentationController::class, 'export'])
                    ->name('admin.documentation.export');
            });

            Route::get('/access-logs', [AdminController::class, 'accessLogsPage'])
                ->name('admin.access-logs');

            Route::get('/activity-logs', [AdminController::class, 'activityLogsPage'])
                ->name('admin.activity-logs');

            Route::get('/system-health', [AdminController::class, 'systemHealthPage'])
            ...
            Route::post('/check-email', [AdminController::class, 'checkEmailAvailability'])
                ->name('admin.check-email');
        
                
            // Login / Session History
            Route::get('/history', [AdminController::class, 'historyPage'])
                ->name('admin.history');
            Route::get('/admin-login-history/{adminId?}', [AdminController::class, 'adminLoginHistory'])
                ->name('admin.login-history');
            Route::post('/clean-old-sessions', [AdminController::class, 'cleanOldSessions'])
                ->name('admin.clean-sessions');
        

            // Technician Management Routes
            Route::get('/service/technicians-grid',[\App\Http\Controllers\ServiceRequestController::class, 'techniciansGrid'])
                ->name('admin.service.technicians-grid');

            Route::get('/service/technicians-list',[\App\Http\Controllers\ServiceRequestController::class, 'techniciansList'])
                ->name('admin.service.technicians-list');

            Route::get('/service/technicians/{id}',[\App\Http\Controllers\ServiceRequestController::class, 'technicianShow'])
                ->name('admin.service.technicians.show');

            Route::put('/service/technicians/{id}',[ServiceRequestController::class, 'technicianUpdate'])
                ->name('admin.service.technicians.update');
            
            Route::delete('/service/technicians/{id}',[\App\Http\Controllers\ServiceRequestController::class, 'technicianDestroy'])
                ->name('admin.service.technicians.destroy');


            // END OF OWNER ONLY ROUTES
        });

        /*
        |--------------------------------------------------------------------------
        | OWNER AND MARKETING ONLY ROUTES
        |--------------------------------------------------------------------------
        */

        Route::middleware([
            \App\Http\Middleware\RoleMiddleware::class . ':owner,marketing'
        ])->group(function () 
        {
            Route::get('/landingpage-tables', [AdminController::class, 'landingPageTables'])
                ->name('admin.landingPageTables');

            Route::get('/subsection-tables', [AdminController::class, 'subsectionTables'])
                ->name('admin.subsectionTables');
                
            Route::get('/subsections/{id}', [AdminController::class, 'subsectionTables'])
                ->name('admin.subsection_tables');

            Route::get('/products-tables', [AdminController::class, 'productsTables'])
                ->name('admin.productsTables');

            Route::get('/promotion', [TableController::class, 'show'])
                ->defaults('type', 'promotion')
            ...
            // Table routes
            Route::get('/portofolio', [TableController::class, 'show'])
                ->defaults('type', 'portofolio')->name('admin.portofolio');

            Route::get('/instagram', [TableController::class, 'show'])
                ->defaults('type', 'instagram')->name('admin.instagram');

            Route::get('/tiktok', [TableController::class, 'show'])
                ->defaults('type', 'tiktok')->name('admin.tiktok');

            Route::get('/testimonials', [TableController::class, 'show'])
                ->defaults('type', 'testimonials')->name('admin.testimonials');

            Route::get('/promotion', [TableController::class, 'show'])
                ->defaults('type', 'promotion')->name('admin.promotion');

            Route::get('/categories', [TableController::class, 'show'])
                ->defaults('type', 'categories')->name('admin.categories');
            
            // CRUD routes for section content
            Route::prefix('section-content')->group(function () 
            {
                Route::post('/', [TableController::class, 'store'])
                    ->name('admin.section-content.store');

                Route::get('/{id}/edit', [TableController::class, 'edit'])
                    ->name('admin.section-content.edit');

                Route::put('/{id}', [TableController::class, 'update'])
                    ->name('admin.section-content.update');

                Route::delete('/{id}', [TableController::class, 'destroy'])
                    ->name('admin.section-content.destroy');
            });

            // END OF OWNER AND MARKETING ONLY ROUTES
        });
        ...
        */
        ...
        Route::middleware([
            \App\Http\Middleware\RoleMiddleware::class . ':owner,support'
        ])->group(function () 
        {
            /*
            |--------------------------------------------------
            | Messages (Core Support Job)
            |--------------------------------------------------
            */
            ...
            Route::get('/sent/{response}', [ContactController::class, 'showSentMessage'])
                ->name('admin.messages.sent.show');

            Route::post('/messages/{message}/read', [ContactController::class, 'markAsRead'])
                ->name('admin.messages.mark-read');

            Route::post('/messages/{message}/assign', [ContactController::class, 'assign'])
                ->name('admin.messages.assign');

            Route::post('/messages/{message}/respond', [ContactController::class, 'respond'])
                ->name('admin.messages.respond');

            Route::post('/messages/{message}/important', [ContactController::class, 'toggleImportant'])
                ->name('admin.messages.toggle-important');

            Route::post('/messages/{message}/trash', [ContactController::class, 'moveToTrash'])
                ->name('admin.messages.trash');

            Route::post('/messages/{message}/restore', [ContactController::class, 'restoreFromTrash'])
                ->name('admin.messages.restore');

            Route::delete('/messages/{message}', [ContactController::class, 'destroy'])
                ->name('admin.messages.destroy');

            Route::delete('/messages/{message}/permanent', [ContactController::class, 'permanentDelete'])
                ->name('admin.messages.permanent-delete');

            Route::post('/messages/read-all', [ContactController::class, 'markAllAsRead'])
                ->name('admin.messages.mark-all-read');
            ...
            */
            ...
            Route::post('/chat/start', [App\Http\Controllers\Admin\ChatController::class, 'startConversation'])
                ->name('admin.chat.start');

            Route::get('/chat/conversation/{conversationId}', [App\Http\Controllers\Admin\ChatController::class, 'getConversation'])
                ->name('admin.chat.conversation');

            Route::post('/chat/conversation/{conversationId}/reply', [App\Http\Controllers\Admin\ChatController::class, 'sendReply'])
                ->name('admin.chat.reply');

            Route::post('/chat/conversation/{conversationId}/resolve', [App\Http\Controllers\Admin\ChatController::class, 'resolveConversation'])
                ->name('admin.chat.resolve');

            Route::post('/chat/conversation/{conversationId}/transfer', [App\Http\Controllers\Admin\ChatController::class, 'transferConversation'])
                ->name('admin.chat.transfer');

            Route::get('/chat/stats', [App\Http\Controllers\Admin\ChatController::class, 'getStats'])
            ...
            |--------------------------------------------------
            */
            Route::get('/faq', [AdminController::class, 'faqPage'])
                ->name('admin.faq');
        });

        /*
        |--------------------------------------------------------------------------
        | OWNER AND TECHNICIAN ONLY ROUTES
        |--------------------------------------------------------------------------
        

        Route::middleware(
            \App\Http\Middleware\RoleMiddleware::class . 'role:owner,technician'
        )->group(function () 
        {
            Route::get('/service/technicians-grid',[\App\Http\Controllers\Admin\ServiceController::class, 'techniciansGrid'])
                ->name('admin.service.technicians-grid');
