    ...
    <div class="navbar-header">
        ...
        <div class="d-flex">
                ...
                <div class="dropdown-menu dropdown-menu-lg dropdown-menu-end p-0"
                    ...
                    <div data-simplebar style="max-height: 230px;">
                        @forelse($recentMessages as $message)
                            ...
                            @endphp
                            <a href="{{ $recentKey ? route('admin.messages.show', $recentKey) : '#' }}" class="text-reset notification-item">
                                <div class="d-flex">
                                    <div class="flex-shrink-0 avatar-sm me-3">
                                    ...
                                    <div class="flex-grow-1">
                                        <h6 class="mb-1">{{ $message->customer->name ?? 'Unknown Customer' }}</h6>
                                        <div class="font-size-13 text-muted">
                                            <p class="mb-1">{{ Str::limit($message->content, 50) }}</p>
                                            <p class="mb-0">
                                                <i class="mdi mdi-clock-outline"></i>
                                                <span>{{ optional(is_object($message) ? $message->created_at : ($message['created_at'] ?? null))->diffForHumans() }}</span>
                                            </p>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="dropdown d-inline-block">
                ...
                data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <span class="d-none d-xl-inline-block ms-1 fw-medium">{{ auth('admin')->user()->user_name ?? 'Admin' }}</span>
    ...
    <div data-simplebar class="h-100">
        ...
        <div id="sidebar-menu">
            ...
            @foreach(config('admin_menu') as $item)
                ...
                {{-- SIMPLE MENU ITEM --}}
                @if(!isset($item['children']) && in_array($role, $item['roles']))
                    <li class="{{ $currentRoute === $item['route'] ? 'mm-active' : '' }}">
                        <a href="{{ route($item['route'], $item['params'] ?? []) }}">
                            <i class="{{ $item['icon'] }}"></i>
                            <span>{{ Str::title(str_replace('_',' ',$item['key'])) }}</span>
                        </a>
                    </li>
                @endif
                ...
                @if(isset($item['children']) && in_array($role, $item['roles']))
                    ...
                    <li class="{{ $isParentActive ? 'mm-active' : '' }}">
                        <a href="javascript:void(0);" class="has-arrow">
                            <i class="{{ $item['icon'] }}"></i>
                            <span>{{ Str::title(str_replace('_',' ',$item['key'])) }}</span>
                        </a>

                        <ul class="sub-menu" aria-expanded="{{ $isParentActive ? 'true' : 'false' }}">
                            ...
                            @foreach($item['children'] as $child)
                                @if(in_array($role, $child['roles']))
                                    ...
                                    @if(!isset($child['children']))
                                        <li class="{{ $currentRoute === $child['route'] ? 'mm-active' : '' }}">
                                            <a href="{{ route($child['route'], $child['params'] ?? []) }}">
                                                {{ Str::title(str_replace('_',' ',$child['key'])) }}
                                            </a>
                                        </li>
                                    @endif

                                    {{-- CHILD WITH SUB-SUBMENU --}}
                                    @if(isset($child['children']))
                                        @php
                                        ...
                                        <li class="{{ $isChildActive ? 'mm-active' : '' }}">
                                            <a href="javascript:void(0);" class="has-arrow">
                                                {{ Str::title(str_replace('_',' ',$child['key'])) }}
                                            </a>

                                            <ul class="sub-menu" aria-expanded="{{ $isChildActive ? 'true' : 'false' }}">
                                                @foreach($child['children'] as $sub)
                                                    @if(in_array($role, $sub['roles']))
                                                        <li class="{{ $currentRoute === $sub['route'] ? 'mm-active' : '' }}">
                                                            <a href="{{ route($sub['route'], $sub['params'] ?? []) }}">
                                                                {{ Str::title(str_replace('_',' ',$sub['key'])) }}
                                                            </a>
                                                        </li>
                                                    @endif
                                                @endforeach
