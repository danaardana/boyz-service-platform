                        ...
                        <ul class="message-list">
                            ...
                            @php
                                ...
                                if (is_object($message) && method_exists($message, 'getKey')) {
                                    ...
                                    $messageKey = $message['k3_id'] ?? null;
                                } else {
                                    $messageKey = $message->k3_id ?? null;
                                }
                            @endphp
                            <li class="{{ !$message->is_read ? 'unread' : '' }} {{ $message->is_important ? 'important' : '' }}" data-message-id="{{ $messageKey }}">
                                <div class="col-mail col-mail-1">
                                    <div class="checkbox-wrapper-mail">
                                        <input type="checkbox" id="chk{{ $messageKey }}" value="{{ $messageKey }}">
                                        <label for="chk{{ $messageKey }}" class="toggle"></label>
                                    </div>
                                    @if($messageKey)
                                        <a href="{{ route('admin.messages.show', $messageKey) }}" class="title">{{ $message->customer?->name ?? 'Unknown' }}</a>
                                    @else
                                        <span class="title">{{ $message->customer?->name ?? 'Unknown' }}</span>
                                    @endif
                                ...
                                <div class="col-mail col-mail-2">
                                    @if($messageKey)
                                        <a href="{{ route('admin.messages.show', $messageKey) }}" class="subject">
                                    @else
