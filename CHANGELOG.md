# 📋 Boyz Projects - Release Notes & Changelog

## **📧 [v2.3.0] - Febuary 2026 – Admin Navigation & Role System Upgrade

This release focuses on improving the internal structure of the admin panel by introducing a dynamic navigation system and a formal role-based access layer for administrators.

---

### 🧭 Admin Navigation System Refactor

The admin sidebar has been fully redesigned to use a configuration‑driven structure instead of hardcoded Blade menu items.

#### What Changed

* Sidebar menu is now generated from `config/admin_menu.php`
* Removed large static navigation blocks from Blade templates
* Menu rendering is now dynamic and centralized

#### New Capabilities

* Supports **multi-level navigation** (menu → submenu → sub-submenu)
* Supports **Laravel routes with parameters** (e.g., documentation pages)
* Automatically detects the active route and highlights the correct menu
* Parent menus auto-expand when a child route is active

#### Benefits

* Easier to maintain and extend
* No need to edit Blade files when adding new menu items
* Cleaner separation between UI structure and application logic

---

### 👤 Admin Role Management System

A formal role system has been introduced to control access and UI visibility within the admin panel.

#### Database Changes

* Added a **required `role` column** to the admin table
* Existing admin records must now have an assigned role
...
| **Marketing** | Products, landing pages, promotions |
| **Support**   | Messages, chat, documentation       |

#### Application Changes

* Admin create form now includes role selection
* Admin edit form allows updating role
* Backend validation ensures only valid roles are saved
* Sidebar menu items now appear based on the logged-in admin’s role

---

### 🔐 Role-Based Interface Behavior

UI components are now conditionally displayed depending on the administrator’s role.

Examples:

* Marketing cannot see system configuration sections
* Support cannot access product or promotion management
* Only Owner can access full user management

This improves both **security** and **usability** by reducing unnecessary access.

---

### 🔧 Codebase Improvements

* Simplified navigation Blade logic using reusable loops
* Centralized permission-based rendering in one config file
* Reduced duplication across layout files
* Prepared system for future module expansion without layout refactors

---

### 📌 Upgrade Notes

If deploying this version on an existing system:

1. Ensure the admin table includes the new `role` column
2. Assign a valid role to all existing admin accounts
3. Clear configuration cache after deployment
...
## **📧 [v2.1.3] - Febuary 2026 – Security Obfuscation Updates


**Release Date:** February 2026

This release focuses on strengthening application security by introducing an obfuscated database schema and improving runtime safety across models, views, and controllers.

---

### 🔐 Database Obfuscation Layer

Core application tables have been migrated to an obfuscated structure to protect sensitive data and reduce direct schema readability.

#### Key Improvements

* Critical tables renamed to non‑descriptive identifiers
* Column names replaced with obfuscated equivalents
* Sensitive fields encrypted at rest
* Backward compatibility maintained through model accessors and mutators

This ensures data remains secure while preserving normal application behavior.

---

### 🧩 Model Refactoring

Multiple Eloquent models were updated to align with the new obfuscated schema.

#### Changes Made

* Updated `$primaryKey` values to match obfuscated IDs
* Adjusted relationships to use new foreign key mappings
* Added accessors and mutators to transparently encrypt and decrypt data
* Ensured timestamps reference new column names where applicable

These changes allow the application to continue functioning without exposing raw schema details.

---

## 🛠 View & Routing Compatibility Fixes

Because identifiers and fields changed, several Blade views and route calls were updated for safety and compatibility.

#### Fixes Included

* Replaced direct property access with safe key resolution
* Guarded `route()` calls where related models may be null
* Updated notification and message links to use obfuscated identifiers
* Ensured timestamps are safely formatted only when values exist

---

### ⚠️ Null-Safety Enhancements

Runtime stability was improved by handling cases where timestamps or related records may be null.

#### Improvements

* Replaced direct `->diffForHumans()` calls with null-safe checks
* Prevented crashes in notifications, admin notifications, and messaging views
* Added conditional formatting for optional relationships

These updates eliminate common runtime exceptions in production.

---

### 📦 New Secure Tables Introduced

Additional system tables were introduced using the obfuscation pattern.

| Table Purpose         | Security Enhancement                          |
| --------------------- | --------------------------------------------- |
| Notifications         | Encrypted message data and metadata           |
| Sessions              | Encrypted payload, IP address, and user agent |
| Password Reset Tokens | Obfuscated columns with custom broker support |
| Products & Options    | Encrypted product content and configuration   |

Backfill commands were added to encrypt existing data in these tables.

---

### 🔧 System Stability Improvements

* Improved compatibility between encrypted data and API responses
* Ensured legacy records are handled safely during transition
* Strengthened error handling for missing or partially migrated records
...
1. Run all new migrations related to obfuscated tables
2. Execute any provided backfill/encryption commands
